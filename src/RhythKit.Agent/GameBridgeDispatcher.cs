...
internal sealed class GameBridgeDispatcher : IDisposable
{
    ...
    private async Task HandleAsync(GameBridgeMessage message)
    {
        ...
        var captureReady = message.MapCaptureReady ?? !string.IsNullOrWhiteSpace(message.MapId);
        GameConnectionState.Update(new GameConnectionUpdate(message.Running ?? true, message.IntegrationConnected ?? true, captureReady, game, version, message.MapId, message.Event));
        ...
        {
            GameConnectionState.Update(new GameConnectionUpdate(true, true, captureReady, game, version, message.MapId, "AuthenticationRequired"));
            return;
        }
        ...
        try
        {
            ...
            GameConnectionState.AddDebug(response.IsSuccessStatusCode ? "ScoreSubmitted" : "ScoreRejected", response.IsSuccessStatusCode ? "Completion submitted to Rhythians." : $"Rhythians rejected the completion: {body}", message.MapId, new { statusCode = (int)response.StatusCode });
            GameConnectionState.Update(new GameConnectionUpdate(true, true, captureReady, game, version, message.MapId, response.IsSuccessStatusCode ? "ScoreSubmitted" : "ScoreRejected"));
        }
        catch (Exception exception)
        {
            GameConnectionState.AddDebug("ScoreSubmissionFailed", exception.Message, message.MapId);
            GameConnectionState.Update(new GameConnectionUpdate(true, false, captureReady, game, version, message.MapId, "ScoreSubmissionFailed"));
        }
    }
